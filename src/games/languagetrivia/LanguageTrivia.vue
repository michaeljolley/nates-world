...
import { getQuestionsForGame, grades, gradeLabels } from './questions.js'
...
const selectedGrade = ref('kindergarten')
const questions = ref([])
...
// Map grade names to numbers for database (unique IDs 200-212 for language trivia)
const gradeMap = {
  'kindergarten': 200, '1st': 201, '2nd': 202, '3rd': 203, '4th': 204, '5th': 205,
  '6th': 206, '7th': 207, '8th': 208, '9th': 209, '10th': 210, '11th': 211, '12th': 212
}

// Number of questions per grade (10 for all)
...
const currentStreak = computed(() => {
  const level = gradeMap[selectedGrade.value]
...
function loadQuestions() {
  ...
  try {
    questions.value = getQuestionsForGame(selectedGrade.value, questionsPerGrade)
  } catch (e) {
...
async function endGame() {
  ...
  const isWin = score.value >= Math.ceil(questionsPerGrade * 0.8)
...
<template>
  <div class="trivia-wrapper" :data-grade="selectedGrade">
    <div class="lightning-bg"></div>
    <div class="bolt bolt-1">📝</div>
    <div class="bolt bolt-2">📝</div>
    
    <div class="container">
      <h1>📚 LANGUAGE TRIVIA ✏️</h1>
      
      <!-- Menu Phase -->
      <template v-if="gamePhase === 'menu'">
        <p class="subtitle">Test your grammar and language arts skills!</p>
        ...
        <div class="rules">
          <h3>How to Play</h3>
          <ul>
            <li>Answer {{ questionsPerGrade }} language arts questions for {{ gradeLabels[selectedGrade] }}</li>
            <li>You have 30 seconds per question</li>
            <li>Score {{ Math.ceil(questionsPerGrade * 0.8) }} or more to continue your streak!</li>
          </ul>
        </div>
      </template>
      ...
      <template v-if="gamePhase === 'results'">
        <div class="results-card">
          ...
          <div class="final-score">
            ...
            <span class="score-total">/ {{ questionsPerGrade }}</span>
          </div>
          
          <p class="score-message" v-if="score === questionsPerGrade">🏆 Perfect Score! Amazing!</p>
          <p class="score-message" v-else-if="score >= Math.ceil(questionsPerGrade * 0.8)">🎉 Great job! Streak continues!</p>
          <p class="score-message" v-else-if="score >= Math.ceil(questionsPerGrade * 0.5)">👍 Good effort!</p>
          <p class="score-message" v-else>📚 Keep studying language arts!</p>
...
.lightning-bg {
  ...
  background: 
    ...
    linear-gradient(-135deg, transparent 40%, rgba(34, 211, 238, 0.03) 50%, transparent 60%);
  z-index: 0;
}
