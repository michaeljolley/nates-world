...
const difficultyMap = { easy: 16, medium: 17, hard: 18 }
...
<template>
  <div class="trivia-wrapper" :data-difficulty="difficulty">
    ...
    <div class="container">
      <h1>📜 HISTORY TRIVIA 🏛️</h1>
      
      <!-- Menu Phase -->
      <template v-if="gamePhase === 'menu'">
        <p class="subtitle">Test your world history knowledge!</p>
        ...
        <div class="rules">
          <h3>How to Play</h3>
          <ul>
            <li>Answer {{ questionsPerDifficulty[difficulty] }} multiple-choice questions about world history</li>
      ...
      <template v-if="gamePhase === 'results'">
        <div class="results-card">
          ...
          <p class="score-message" v-if="score === questionsPerDifficulty[difficulty]">🏆 Perfect Score! You're a history master!</p>
          <p class="score-message" v-else-if="score >= Math.ceil(questionsPerDifficulty[difficulty] * 0.8)">🎉 Great job! Streak continues!</p>
          <p class="score-message" v-else-if="score >= Math.ceil(questionsPerDifficulty[difficulty] * 0.5)">👍 Good effort!</p>
          <p class="score-message" v-else>📚 Keep learning about history!</p>
...
.trivia-wrapper {
  ...
  color: #d4a574;
  text-shadow: 0 0 10px #d4a574, 0 0 20px #8b6914;
...
.lightning-bg {
  ...
  pointer-events: none;
  background: 
    linear-gradient(135deg, transparent 40%, rgba(139, 105, 20, 0.05) 50%, transparent 60%),
    linear-gradient(-135deg, transparent 40%, rgba(212, 165, 116, 0.05) 50%, transparent 60%);
  z-index: 0;
}
...
.trivia-wrapper[data-difficulty="medium"] {
  ...
  --secondary: #b8860b;
  color: #c9a227;
  text-shadow: 0 0 10px #c9a227, 0 0 20px #b8860b;
}
...
.trivia-wrapper[data-difficulty="hard"] {
  ...
  --secondary: #8b4513;
  color: #cd853f;
  text-shadow: 0 0 10px #cd853f, 0 0 20px #8b4513;
}
...
.container::before {
  ...
  right: -3px;
  bottom: -3px;
  background: conic-gradient(from 0deg, #d4a574, #8b6914, #c9a227, #cd853f, #d4a574);
  border-radius: 22px;
  z-index: -1;
...
.streak-display.guest {
  border: 2px solid var(--primary);
  background: linear-gradient(135deg, rgba(212, 165, 116, 0.1) 0%, rgba(139, 105, 20, 0.2) 100%);
  cursor: pointer;
...
.streak-display.guest:hover {
  background: linear-gradient(135deg, rgba(212, 165, 116, 0.2) 0%, rgba(139, 105, 20, 0.3) 100%);
