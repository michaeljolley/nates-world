...
const difficultyMap = { easy: 10, medium: 11, hard: 12 }
...
<template>
  <div class="trivia-wrapper" :data-difficulty="difficulty">
    ...
    <div class="container">
      <h1>📖 BIBLE TRIVIA ✝️</h1>
      
      <!-- Menu Phase -->
      <template v-if="gamePhase === 'menu'">
        <p class="subtitle">Test your Bible knowledge!</p>
        ...
        <div class="rules">
          <h3>How to Play</h3>
          <ul>
            <li>Answer {{ questionsPerDifficulty[difficulty] }} multiple-choice questions about Bible stories</li>
      ...
      <template v-if="gamePhase === 'results'">
        <div class="results-card">
          ...
          <p class="score-message" v-else>📚 Keep learning Bible stories!</p>
...
.lightning-bg {
  ...
  pointer-events: none;
  background: 
    linear-gradient(135deg, transparent 40%, rgba(168, 85, 247, 0.03) 50%, transparent 60%),
    linear-gradient(-135deg, transparent 40%, rgba(251, 191, 36, 0.03) 50%, transparent 60%);
  z-index: 0;
}
...
.container::before {
  ...
  right: -3px;
  bottom: -3px;
  background: conic-gradient(from 0deg, #a855f7, #fbbf24, #f59e0b, #a855f7);
  border-radius: 22px;
  z-index: -1;
...
.streak-display.guest {
  border: 2px solid var(--primary);
  background: linear-gradient(135deg, rgba(168, 85, 247, 0.1) 0%, rgba(251, 191, 36, 0.2) 100%);
  cursor: pointer;
...
.streak-display.guest:hover {
  background: linear-gradient(135deg, rgba(168, 85, 247, 0.2) 0%, rgba(251, 191, 36, 0.3) 100%);
